...
def read_plate(yolo_license_plate, im):
    ...
    # --- PHẦN SỬA ĐỔI CHO YOLOV8 ---
    # Lấy bounding boxes (x1, y1, x2, y2, conf, cls)
    bb_list = results[0].boxes.data.tolist()
    # Lấy map tên class {0: '0', 1: '1', ... 35: 'Z'}
    ...
    for bb in bb_list:
        ...
        # Lấy tên class (ký tự) dựa vào class_id (bb[5])
        class_id = int(bb[5])
        char_name = names[class_id]
        
        center_list.append([x_c, y_c, char_name]) # Lưu char_name thay vì bb[-1]
    ...
    # size = results.pandas().s # Dòng này ko cần thiết nữa
